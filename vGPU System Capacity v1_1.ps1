############################################################################
# vGPU System Capcity for PowerCLI Version 1.1
...
Function vGPUSystemCapacity {
	...
	[string]
	$vGPUType)
	# This function takes a string argument of the vGPU value. It will then calculate the remaining number of that vGPU type that can be deployed and returns it.
	# Example: vGPUSystemCapacity "grid_p4-2q"
	# Will return the number of remaing VMs that can be powered on with that given profiles
	...
	try{
		...
		Get-VMHostPciDevice -deviceClass DisplayController -Name "NVIDIA Corporation NVIDIATesla*" | ForEach-Object {
		...
		#should test for a null object at this point in case the system has no vGPUs
		...
		foreach($vm in Get-vm "*"){
			...
			if($CurrvGPU -match "grid"){
				...
				if ($null -ne $ActivevGPUs -and @($ActivevGPUs).count -gt 0){ #make sure not working with a null array
					...
					#write-Host "loc of vGPU " $LocOfvGPU 
				}
				if($LocOfvGPU -lt 0){
					#write-Host "no match should add an element"
		...
		if ($null -ne $ActivevGPUs -and @($ActivevGPUs).count -gt 0){ #make sure not working with a null array
			...
			foreach($vGPU in $ActivevGPUs){
				#write-Host "vGPU considerd " $vGPU.vGPUname
				#write-Host "chosen vGPU " $MyChosenvGPU
